...
- **Coverage lost from specs that open their own browser context**: `_coverageCollector` read `window.__coverage__` from the Playwright `page` fixture only. A spec that creates its own context in `beforeAll` (`browser.newContext()` / `context.newPage()`) and runs every test there left the fixture page on `about:blank`, so collection found nothing and returned quietly — the run stayed green while all coverage was discarded. In `rhdh-plugin-export-overlays` this silently zeroed three workspaces (`scorecard`, `intelligent-assistant`, `adoption-insights` — 57 passing tests, 0 coverage files) while the instrumented bundles were deployed and exercised normally. Collection now walks every open context, reading through `browser` while still depending on `context` so teardown ordering keeps the fixture's own context open long enough to be read.
- **Coverage files are now written per page instead of per test**: a context opened in `beforeAll` outlives the whole spec file and `__coverage__` accumulates in it, so per-test naming wrote a growing copy of the same report once per test — 34 near-duplicates for the largest workspace, all summed again by `nyc merge`. Files are keyed by page and overwritten, keeping the last and most complete read. This also collapses retries, where a failed and a passing attempt were both counted.
- **Coverage collection can no longer fail a passing test**: writing and browser access run inside a guard, since the collector runs in teardown for every test and an unwritable `outputDir` or a browser that died mid-test would otherwise change a verdict. Problems are reported instead — as a test annotation, visible in the HTML report, plus one console line per distinct message. A single page that crashes mid-collection no longer discards the other pages' coverage.
