...
The package includes default current-frontend plugins. Override their versions or
configuration in `tests/config/dynamic-plugins.yaml` ([guide](/guide/configuration/config-files#dynamic-pluginsyaml)).

Helm-specific overrides belong in `value_file.yaml` and are merged after the
package defaults.
